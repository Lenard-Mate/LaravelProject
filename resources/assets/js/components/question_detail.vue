  ...
  <div>
    <div class="row question-info">
      <div class="col-lg-2">
        <p
          class="text-grey"
          v-on:click="voteQuestion(question.id, constants.vote_type.UP_VOTE)"
        >
          <i
            v-bind:class="{ voted: question.up_voted }"
            class="fa fa-sort-asc fa-3x"
          ></i>
        </p>
        <h3 class="text-grey">{{ question.votes }}</h3>
        <p
          class="text-grey"
          v-on:click="voteQuestion(question.id, constants.vote_type.DOWN_VOTE)"
        >
          <i
            v-bind:class="{ voted: question.down_voted }"
            class="fa fa-sort-desc fa-3x"
          ></i>
        </p>
      </div>
      <div class="col-lg-10">
        ...
        <div>
          <a
            ...
            class="text-brown"
            v-show="userId == question.asked_user_id"
            >edit</a
          >
        ...
        <div class="question-author-info pull-right">
          ...
          <img
            width="60px"
            height="60px"
            v-bind:src="question.asked_user_avatar"
          />
          <a :href="'/user/' + question.asked_user_id + '/view'"
            >&nbsp; {{ question.asked_user }}</a
          >
        </div>
      </div>
    </div>
        ...
        <div v-for="(answer, index) in answers">
          <div class="row">
            <div class="col-lg-2 col-md-2 col-sm-6">
              <p
                class="text-grey"
                v-on:click="
                  voteAnswer(answer.id, constants.vote_type.UP_VOTE, index)
                "
              >
                <i
                  v-bind:class="{ voted: answer.up_voted }"
                  class="fa fa-sort-asc fa-3x"
                ></i>
              </p>
              ...
              <p
                class="text-grey"
                v-on:click="
                  voteAnswer(answer.id, constants.vote_type.DOWN_VOTE, index)
                "
              >
                <i
                  v-bind:class="{ voted: answer.down_voted }"
                  class="fa fa-sort-desc fa-3x"
                ></i>
              </p>
              ...
              <p
                class="text-grey"
                v-show="userId == question.asked_user_id"
                v-on:click="acceptAnswer(answer.id, index)"
              >
                <i
                  v-bind:class="{ accepted: answer.accepted }"
          ...
          <div class="row">
            ...
            >
              <a
                ...
                class="text-brown"
                v-show="userId == answer.user.id"
                >edit</a
              >
              <!-- <a href="" class="text-brown" v-show="userId == answer.user.id">delete</a>                             -->
            </div>
            <div
              class="col-lg-4 col-lg-offset-2 col-md-4 col-md-offset-2 col-sm-6 answer-author-info"
            >
              <p>answered {{ answer.formatted_created_at }}</p>
              <img
                width="60px"
                height="60px"
                v-bind:src="'/uploads/avatars/' + answer.user.avatar"
              />
              <a :href="'/user/' + answer.user.id + '/view'"
                >&nbsp; {{ answer.user.name }}</a
              >
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-lg-offset-2 col-lg-10 add-a-comment">
              <p
                v-show="!answer.showAddComment && isAuthenticated"
                v-on:click="showComment(answer)"
              >
                add a comment
              </p>
...
export default {
  ...
  methods: {
    ...
    fetchAnswers() {
      axios.get("/answers/" + this.question_id).then((res) => {
